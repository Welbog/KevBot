...
/**
 * RAHL is a sorted random access array list with binary search
 * algorithms.
 * <p>
 * It's methods allow it to store strings in case-insensitive
 * alphabetical order, and it is equally good at reading randomized
 * and presorted data.
 *
 * @version 1.0.0
 * @see Kevbot
...
public class RAHL {
  ...
  public synchronized int add(String s) {
    if (size != 0) {
      if (strupSearch(s, lines[size-1]) > 0) { // If it goes at the end, let's not do anything special
        insertAt(size);
        lines[size-1] = s;
        return size-1;
  ...
  /**
   * Do the important of finding the right place to
   * put the String and place it there.
   * Implements a binary search.
   * @param s The String to add.
   * @param low The lower bound of where s needs to be added.
   * @param high The upper bound of where s needs to be added.
   */
  ...
  /**
   * Check to see if a String is present.
   * Implements a binary search.
  ...
  public synchronized boolean exists(String s, int low, int high) {
    ...
    if (strupSearch(s, lines[size-1]) > 0) { // If the value is bigger than the last value, return false immediately.
        return false;
    }
    if (size == 0) { return false; } // Base case
    int guess = (low+high)/2; // Find the middle
    if (guess >= size) { return false; } // We've gone to far!
    if (low > high) { // You maniacs! You blew it up!
      return false; }
    if (matx0r(s, lines[guess])) { return true; } // w00t
  ...
  /**
   * Get the index of a given String contained in the RAHL.
   * For use with RAHL.get(int i).
   * Implements a binary search.
   * @param s The String to look for.
   * @param low The starting point.
   * @param high The ending point.
   * @return The index corresponding to the given String, or -1 if it doesn't match.
   */
  public synchronized int getIndex(String s, int low, int high) {
      //s = s.trim();
      if (size == 0) { return -1; } // Oh crap!
      int guess = (low+high)/2; // Find the middle.
      if (guess >= size) { return -1; } // OH FUCK!
      if (low > high) { // SHIT!
          return -1; }
      if (matx0r(s, lines[guess])) { return guess; } // w00t
   ...
   * returns the closest String in the RAHL that matches the String 
   * provided and return that.
   * Implements a binary search.
   * @param s The String to look for.
   * @param low The starting point.
   * @param high The ending point.
  ...
  public synchronized String getRealString(String s, int low, int high) {
      if (size == 0) { return s; } // Oops.
      int guess = (low+high)/2; // Find the middle.
      if (guess >= size) {return s; } // We overshot!
      //if (s.substring(0, Math.min(lines[guess].length(),s.length())).equalsIgnoreCase(lines[guess])) { return lines[guess]; }
      if (matx0r(s, lines[guess])) { return lines[guess]; } // w00t.
      if (low > high) { // DAMN!
          return s; }
   ...
   * @param i The index of the String you want.
   ...
   * @param x The index to collapse around.
   */
   ...
   * @param x The index at which a String will be inserted.
   */
  private void insertAt(int x) {
    //System.out.println("size: " + size + " lines.length: " + lines.length + " x: " + x);
