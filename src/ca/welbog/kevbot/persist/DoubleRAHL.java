 ...
 * <p>
 * It stores a RAHL of Strings and uses the indices of the stored Strings
 * to index another array of Strings. The Strings stored in the RAHL are 
 * called "searchable Strings" and are also called messages. The Strings
 * stored in the array are called "indexed Strings" and are also called
 * replies.
 * <p>
...
public class DoubleRAHL {
  ...
  // Author's note: This RAHL is called thomas for reasons beyond your mortal understanding.
  ...
  /**
   * Add a message and a reply to the DoubleRAHL.
   * This method implements a binary search algorithm.
   * @param m The searchable String (message) to be added.
   * @param reply The indexed String (reply) to be added.
   */
  public synchronized void add(String m, String reply) {
    //System.err.println("DoubleRAHL::add("+m+"," +reply+ ");");
    if ((thomas.getSize()!=0) && (thomas.exists(m, 0, thomas.getSize()))) { // If there's something there and m exists.
   ...
   * @param m The searchable String (message) to be appended to.
   * @param reply The indexed String (reply) to be appended.
   */
   ...
   * @param m The searchable String (message) to be changed.
   * @param r The indexed String (reply) to be added.
   ...
   */
  public synchronized int getSize() { return thomas.getSize(); }

  /**
   * Find out if s is contained in the DoubleRAHL.
   * This implements a binary search algortihm.
   * @param s The searchable String (message) to search for.
  ...
  public synchronized boolean exists(String s, int low, int high) {
    ...
    /**
   * Find the index of a String in the DoubleRAHL.
   * This implements a binary search algortihm.
   * @param s The searchable String (message) to search for.
   * @param low The starting index.
   * @param high The ending index.
  ...
  public synchronized int getIndex(String s, int low, int high) {
    ...
    /**
   * Remove a String from the DoubleRAHL.
   * This implements a binary search algortihm.
   * @param s The searchable String (message) to search for.
   */
  public synchronized void delete(String s) {
      if (getSize() == 0) { return; } // If there's nothing there, we can't do anything.
      int x = thomas.getIndex(s, 0, thomas.getSize()); // Find where this thing is.
   ...
   * @param i The index of the reply wanted.
  ...
  /**
   * Get a reply by message.
   * Given a searchable String, return the corresponding indexed String.
   * This method implements a binary search alorithm.
  ...
  public synchronized String getReply(String m) {
      if (getSize() == 0) {return "";}
      int x = thomas.getIndex(m, 0, thomas.getSize());//THE MEEK!!! indeed /*NOT THE MEKE!!!*/
      if (x >= 0) {
          return replies[x];
      }
      return "";
   ...
   * @param i The index of the message wanted.
   ...
   * @param s The searchable String to match.
   ...
   * @param x The index that is to be destroyed.
   */
  ...
  /**
   * Helper method does the opposite of collapse(int x): it opens a space at a given index.
   * @param x The index into which you want to insert an element.
   */
  private void insertAt(int x) {
    //System.out.println("size: " + thomas.getSize() + " lines.length: " + replies.length + " x: " + x);
