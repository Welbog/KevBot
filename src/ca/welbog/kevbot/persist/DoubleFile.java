...
public class DoubleFile implements DoubleStorage {
  String messageFile; // The file in which we store the messages for which to look. (sorted file)
  String replyFile; // The file in which we store the replies to the messages. (indexed file)
  DoubleRAHL replies; // The DoubleRAHL corresponding to the database of messages and replies.

  /**
   * Constructor builds a DoubleFile and fills it with the information in the filenames provided.
   * @param g The file from/into which messages will be read/stroed (searchable).
   * @param r The file from/into which replies will be read/stored (indexed).
   */
  public DoubleFile(String g, String r) {
    messageFile = g;
    replyFile = r;
    replies = new DoubleRAHL(); // Create a DoubleRAHL to store our Strings.
    fillLines(); // Fill the files if they exist, create the files if they do not.
  }
  
  /**
   * Read all Strings from the files if they exist, create the files if they do not.
   */
