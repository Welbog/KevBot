...
public class NamesWithSpacesUtils {
  ...
  /**
   * This is a method that converts a single-word name into a multi-word name if
   * there are quotes around the name.
   * 
   * @param firstWord
   * @param remainingWords
   * @return
   */
  public static NameAndRemainder getNameWithSpaces(String firstWord,
    ...
    if (!firstWord.matches("^\".*")) { // We only care about this if the first
                                       // word is quoted.
    ...
    for (; i < remainingWords.size(); i++) {
      ...
      if (word.matches(".*\"$")) {
        result.name += " " + word.substring(0, word.length() - 1); // Remove the
                                                                   // quote
        break;
      }
      else {
    ...
    result.remainder = new Vector<>(remainingWords.subList(i + 1, remainingWords.size())); // Remove
                                                                                           // the
                                                                                           // name
