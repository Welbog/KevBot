...
public class HelpResponder implements Responder {
  private Map<String,ResponderMetadata> responders = new HashMap<String,ResponderMetadata>();
  
  public HelpResponder(List<ResponderMetadata> responders) {
    for (ResponderMetadata r : responders) {
  ...
  public Response getResponse(Request r) {
    ...
    if (details == null) {
      ...
      if (responders.get(detail) == null) {
        if (
          detail.equalsIgnoreCase("admin") ||
          detail.equalsIgnoreCase("unadmin") ||
          detail.equalsIgnoreCase("ignoreuser") ||
          detail.equalsIgnoreCase("unignoreuser") ||
          detail.equalsIgnoreCase("ignoreword") ||
          detail.equalsIgnoreCase("unignoreword") ||
          detail.equalsIgnoreCase("chgnick") ||
          detail.equalsIgnoreCase("join") ||
          detail.equalsIgnoreCase("leave") ||
          detail.equalsIgnoreCase("quit")
        ) {
  ...
  private String getCommandList() {
    ...
    for (Map.Entry<String,ResponderMetadata> entry : responders.entrySet()) {
      String alias = entry.getKey();
      if (entry.getValue().isAdminOnly()) { alias += "*"; }
      aliases.add(alias);
    }
    aliases.add("admin*");
    aliases.add("unadmin*");
  ...
  private String getDetails(String alias, String name) {
    ...
    ResponderMetadata metadata = responders.get(alias);
    if (metadata != null) {
      return metadata.getDocumentation().getBody();
    }
    ...
    if (alias.equalsIgnoreCase("addressing")) {
      ...
      return "Syntax: admin <USERNAME>\n" + 
             "Allow a user access to " + name + "'s special commands.\n" +
      ...
      return "Syntax: <MESSAGE> is/are <REPLY> `<SUBEXPRESSION>`\n" +
             name + " will interpret subexpressions in factoids that are delimited by backticks (`).\n" +
