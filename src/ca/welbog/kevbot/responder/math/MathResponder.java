...
public class MathResponder implements Responder {
  ...
  public Response getResponse(Request r) {
    ...
    for (int i = 0; i < body.length(); i++) {
        if (body.charAt(i) != '@') {
            ...
            KarmaFile karma = new KarmaFile("karma.txt","karmb.txt"); // Read-only karma file
    ...
    if (!r.canReply()) { return null; } // We still want to define functions/variables if we shouldn't reply.
    
    String output = "";
    for (MathParser2 math : parsers) {
      if (math.getFail()) { continue; }
    
      String result = math.getValue();
      if (result.equalsIgnoreCase("nan")) { result = "Nullity"; } // Transreal math
      if (output.isEmpty()) {
        output = result;
      }
      else {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(responder.getResponse(
        new Request("irc","#test", "user", "[$dog=5]wa[$dog]", "KevBot", StatefulBot.Mode.STANDARD, Request.Type.MESSAGE)
        ).getBody());
  }
