...
public class DiceResponder implements Responder {
  ...
  public Documentation getDocumentation() {    
    ...
    return new Documentation(
        "Syntax: ndm or nxm, where n and m are whole numbers.\n"+
        ...
        "NOTE: More than " + MAX_DICE + " dice, or dice with more than " + MAX_FACES + " are not permitted."+
        "",
        aliases
    );
  }
  ...
  public Response getResponse(Request r) {
    ...
    if (!body.matches("\\d*[xd]\\d+")) { return null; }
    ...
    if (x > MAX_DICE) {
      return new Response(r.getChannel(),"Error: " + r.getSender() + ", number of dice to roll cannot be greater than " + MAX_DICE + ".",Type.MESSAGE);
    }
    if (y > MAX_FACES) {
      return new Response(r.getChannel(),"Error: " + r.getSender() + ", number of dice faces cannot be greater than " + MAX_FACES + ".",Type.MESSAGE);
    }
    ...
    for (int i = 0; i < x + explosions; i++) {
      ...
      m += temp + " ";
      if (explosions > MAX_DICE) { break; }
    }
    String explodeResponse = "";
    if (explode) {
      explodeResponse = "("+explosions+" explosions) ";
    }
    return new Response(r.getChannel(),r.getSender() + " rolled " + m + explodeResponse + "Total: " + result + "",Type.MESSAGE);
  }
