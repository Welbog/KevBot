...
public class SQLWeightedMarkov {
  ...
  private Connection getConnection() {
	...
	public static void main(String[] args) {
		SQLWeightedMarkov m1 = new SQLWeightedMarkov(1);
		SQLWeightedMarkov m2 = new SQLWeightedMarkov(2);
	...
	*/
	...
	private synchronized void writePair(String seed, String word) {
      ...
      Connection conn = getConnection();
         ...
         ResultSet countSet = statement.executeQuery("SELECT COUNT(*) FROM weightedmarkov WHERE seed = '"+seed+"' AND word = '"+word+"';");
         countSet.first();
         int count = countSet.getInt(1);
         countSet.close();
         if (count > 0) {
           statement.execute("UPDATE weightedmarkov SET weight = weight+1 WHERE seed = '"+seed+"' AND word = '"+word+"';");
         }
         else {
           statement.execute("INSERT INTO weightedmarkov (seed, word, weight) VALUES ('"+seed+"','"+word+"',1);");
         }
         statement.close();
	...
	private synchronized String retrieveNext(String seed) {
      ...
      Connection conn = getConnection();
			   ...
			   ResultSet countSet = statement.executeQuery("SELECT SUM(weight) FROM weightedmarkov WHERE seed = '"+seed+"';");
			   countSet.first();
			   int count = countSet.getInt(1);
			   countSet.close();
			   ...
			   ResultSet rs = statement.executeQuery("SELECT word, weight FROM weightedmarkov WHERE seed = '"+seed+"';");
