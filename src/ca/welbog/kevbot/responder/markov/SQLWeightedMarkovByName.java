...
public class SQLWeightedMarkovByName {
  ...
  public synchronized String generateSentence(String user, Vector<String> tokens,
      boolean useSeedInOutput) {
  ...
  /*
   * public static void main(String[] args) { SQLWeightedMarkovByName m1 = new
   * SQLWeightedMarkovByName(1); SQLWeightedMarkovByName m2 = new
   * SQLWeightedMarkovByName(2);
   * 
   * System.out.println(m1.generateSentence("Inferno"));
   * 
   * List<String> users = Arrays.asList(new String[]
   * {"Inferno","Nick","Agent","Blick_Winkel","Abent"}); long time =
   * System.currentTimeMillis();
   * System.out.println(m2.determineProbability("Nick is a butt.",users));
   * System.out.println(System.currentTimeMillis() - time); time =
   * System.currentTimeMillis(); System.out.println(m2.
   * determineProbability("I am an archeologist, but I said frogs aren't birds."
   * ,users)); System.out.println(System.currentTimeMillis() - time); time =
   * System.currentTimeMillis();
   * System.out.println(m2.determineProbability("Nick is a butt.",users));
   * System.out.println(System.currentTimeMillis() - time); time =
   * System.currentTimeMillis(); System.out.println(m2.
   * determineProbability("My favourite dog ate every sandwich in the universe."
   * ,users)); System.out.println(System.currentTimeMillis() - time); time =
   * System.currentTimeMillis();
   * System.out.println(m2.determineProbability("you are a butt",users));
   * System.out.println(System.currentTimeMillis() - time); time =
   * System.currentTimeMillis(); System.out.println(m2.
   * determineProbability("sounds like your new function was a failure!",users))
   ...
   * List<String> list = new LinkedList<String>(); StringTokenizer tokenizer =
   * new StringTokenizer(sentence); while (tokenizer.hasMoreTokens()) {
   * list.add(tokenizer.nextToken()); } return determineProbability(list,users);
   * }
   */
  ...
  public synchronized String determineProbability(List<String> tokens, List<String> users) {
    ...
    if (bestUser.equalsIgnoreCase("")) {
      ...
      return "I am " + format.format(bestProbability * 100.0) + "% confident that " + bestUser
          + " said, \"" + join(tokens).trim() + "\"";
    }
  }
  ...
  private synchronized String determineProbability(List<String> tokens, String user,
      Hashtable<String, Double> globalTable) {
    if (tokens == null) {
      return "";
    }
    if (tokens.size() <= 1) {
    ...
    DecimalFormat format = new DecimalFormat("#.##");
    return "I am " + format.format(weight * 100.0) + "% confident that " + user + " said, \""
        + join(tokens).trim() + "\"";
  }

  /*
   * Returns a hash table indexed by GLOBAL + seed, with the weight of that
   * seed/word pair
   */
  private synchronized Hashtable<String, Double> determineProbability(List<String> tokens) {
    ...
    for (String word : tokens) {
      String seed = queueToToken(currentToken);
      // System.out.println("Getting weight for " + user + "," + seed + "," +
      // word);
  ...
  /*
   * Returns a hash table indexed by user + seed, with the weight of that
   * seed/word pair for that user
   */
  private synchronized double determineProbabilityByName(List<String> tokens, String user,
      Hashtable<String, Double> globalTable) {
    if (tokens == null) {
      return 0.0;
    }
  ...
  public synchronized List<String> getNotableUsers() {
    ...
    try {
      ...
      ResultSet countSet = statement.executeQuery(
          "select user, count(1) from weightedmarkovbyuser group by user order by 2 desc limit 20;");
  ...
  private synchronized void writePair(String user, String seed, String word) {
    try {
      ...
      ResultSet countSet = statement
          .executeQuery("SELECT COUNT(*) FROM weightedmarkovbyuser WHERE user = '" + user
              + "' AND seed = '" + seed + "' AND word = '" + word + "';");
      countSet.first();
      int count = countSet.getInt(1);
      countSet.close();
      if (count > 0) {
        statement.execute("UPDATE weightedmarkovbyuser SET weight = weight+1 WHERE user = '" + user
            + "' AND seed = '" + seed + "' AND word = '" + word + "';");
      }
      else {
        statement.execute("INSERT INTO weightedmarkovbyuser (user, seed, word, weight) VALUES ('"
            + user + "','" + seed + "','" + word + "',1);");
      }
      statement.close();
    }
  ...
  private synchronized double getWeight(String seed, String word) {
    try {
      ...
      /*
       * ResultSet countSet = statement.
       * executeQuery("SELECT SUM(weight) FROM weightedmarkov WHERE user = '"
       * +user+"' AND seed = '"+seed+"';"); countSet.first(); int count =
       * countSet.getInt(1); countSet.close();
       * 
       * if (count == 0) { statement.close(); return 0.0; }
       */
      ResultSet rs = statement.executeQuery("SELECT weight FROM weightedmarkov WHERE seed = '"
          + seed + "' AND word = '" + word + "';");
  ...
  private synchronized double getWeightByUser(String user, String seed, String word) {
    try {
      ...
      /*
       * ResultSet countSet = statement.
       * executeQuery("SELECT SUM(weight) FROM weightedmarkovbyuser WHERE user = '"
       * +user+"' AND seed = '"+seed+"';"); countSet.first(); int count =
       * countSet.getInt(1); countSet.close();
       * 
       * if (count == 0) { statement.close(); return 0.0; }
       */
      ResultSet rs = statement.executeQuery("SELECT weight FROM weightedmarkovbyuser WHERE user = '"
          + user + "' AND seed = '" + seed + "' AND word = '" + word + "';");
  ...
  private synchronized String retrieveNext(String user, String seed) {
    try {
      ...
      ResultSet countSet = statement
          .executeQuery("SELECT SUM(weight) FROM weightedmarkovbyuser WHERE user = '" + user
              + "' AND seed = '" + seed + "';");
      countSet.first();
      int count = countSet.getInt(1);
      countSet.close();
      ...
      ResultSet rs = statement
          .executeQuery("SELECT word, weight FROM weightedmarkovbyuser WHERE user = '" + user
              + "' AND seed = '" + seed + "';");
