...
public class SQLStrangeMarkovByName {
  ...
  /* Get the total strange weight of a given user/seed:
    SELECT
      w.user,
      w.seed,
      SUM(w.weight / g.weight) AS total
    FROM
      weightedmarkovbyuser w
    INNER JOIN 
      weightedmarkov g
      ON g.seed = w.seed
      AND g.word = w.word
  ...
  /* Get the strange weights of a given user/seed:
    SELECT
      w.user,
      w.seed,
      w.word,
      (w.weight / g.weight) AS weight
    FROM
      weightedmarkovbyuser w
    INNER JOIN 
      weightedmarkov g
    ...
    try {
      Connection conn = getConnection();
         ...
         ResultSet countSet = statement.executeQuery(
             "SELECT w.user, w.seed, SUM(w.weight / g.weight) AS total " +
             "FROM weightedmarkovbyuser w INNER JOIN weightedmarkov g " + 
             "ON g.seed = w.seed AND g.word = w.word " + 
             "WHERE w.user = '"+user+"' AND w.seed = '"+seed+"';"
         );
         countSet.first();
         ...
         ResultSet rs = statement.executeQuery(
             "SELECT w.user, w.seed, w.word, (w.weight / g.weight) AS weight " +
             "FROM weightedmarkovbyuser w INNER JOIN weightedmarkov g " + 
             "ON g.seed = w.seed AND g.word = w.word " +
             "WHERE w.user = '"+user+"' AND w.seed = '"+seed+"';"
         );
         ...
         while (rs.next()) {
           ...
           if (target >= 0 && target < weight) { result = rs.getString("word"); wasSet = true; break; }
           target -= weight;
