...
public class ReplyListenerResponder implements Responder {
  ...
  public Response getResponse(Request r) {
    if (!r.getMessage().matches("(?i)^.*\\s+(is|are)\\s+.*")) { return null; }
    ...
    if (containsIs(first,remainder) < 0) { return null; } 
  ...
  private int containsIs(String firstword, Vector<String> v) {
    ...
    for (int x = start; x < v.size()-1; x++) {
      if ((((String)v.get(x)).equalsIgnoreCase("is")) || (((String)v.get(x)).equalsIgnoreCase("are"))) {
        i = x;
        return i;
      }
    }
   ...
   * @param f The first part of the message (not included in the Vector).
   * @param m The Vector containing the rest of the message.
   * @param v The location of the word "is."
  ...
  private String doAllTheWork(String f, Vector<String> r, int v, String user) {
    ...
    if (replies.exists(m.trim())) {
      if (r.get(v+1).equalsIgnoreCase("|")) { multiple = true; }
  ...
  private void addMultiples(String message, String delimitedReplies, String user, String verb) {
    ...
    String[] multis = delimitedReplies.split("\\s+\\|\\s+"); // Split the request into multiples
    for (String individualReply : multis) {
      if (individualReply.trim().equals("")) { continue; }
