...
public class FactoidResponder implements Responder {
  ...
  public Response getResponse(Request r) {
    ...
    if (!r.getMessage().matches("(?i)^factoid.*")) { return null; }
  ...
  // Helper method for breaking a very long string into a series of shorter strings, each at least MAX_LINE_LENGTH long.
  private static String breakUp(String input) {
    int start = MAX_LINE_LENGTH;
    while (true) {
      // Find the first "|" after 'start' number of characters, and add a newline after it.
      Pattern p = Pattern.compile("^.{" + start + "}[^|]*\\|", Pattern.DOTALL);
      Matcher m = p.matcher(input);
      if (m.find()) {
        input = m.replaceFirst("$0\n");
        start = m.end() + 1 + MAX_LINE_LENGTH; // m.end() is the size of the group that was matched, and we've added one character to that
      }
      else {
        break;
      }
