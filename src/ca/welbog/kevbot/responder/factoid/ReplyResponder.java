...
public class ReplyResponder implements Responder {
  ...
  public Documentation getDocumentation() {
    ...
    return new Documentation(
      ...
      "See also: factoid, forget, quiet, replyadv, recursion, status.",
      aliases
    );
  }
  ...
  public Response getResponse(Request r) {
    ...
    if (reply == null || !reply.hasContent()) { // If all else fails, look for a reply.
      if (r.mustReply()) {
        return new Response(r.getChannel(),"I don't have anything matching " + strup(r.getMessage()) + ", " + r.getSender() + ".",Type.MESSAGE);
      }
      else {
        return null;
      }
  ...
  private Response getReply(String body, String sender, String channel) {
    ...
    String reply = replies.getSingleReply(body); // Find the reply to the message
    
    if (reply.equalsIgnoreCase("")) {return null;} // If it wasn't there, who cares?
